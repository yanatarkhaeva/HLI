...
class Convolution2D:
    # слой свертки
    """
    Слой свёртки включает в себя для каждого канала свой фильтр, ядро свёртки которого обрабатывает предыдущий слой
    по фрагментам (суммируя результаты матричного произведения для каждого фрагмента). Весовые коэффициенты ядра свёртки
    (небольшой матрицы) неизвестны и устанавливаются в процессе обучения.
    Особенностью свёрточного слоя является сравнительно небольшое количество параметров, устанавливаемое при обучении.
    Так например, если исходное изображение имеет размерность
    100×100 пикселей"""  # ------------------- 28*28 = 784
    """по трём каналам (это значит 30000 входных нейронов),"""  # по 3 каналам
    """а свёрточный слой использует"""
    """фильтры c ядром 3x3 пикселя"""  # ---- 5*5
    """с выходом на 6 каналов,"""
    """тогда в процессе обучения определяется только"""
    """9 весов ядра,"""
    """однако по всем сочетаниям каналов, то есть 9×3×6 =162, в таком случае данный слой требует нахождения только"""
    """162 параметров, что существенно меньше количества искомых параметров полносвязной нейронной сети.
    """
    def __init__(self, inputs_channel, num_filters, kernel_size, padding, stride, learning_rate, name):
        ...
        self.bias = np.zeros((self.F, 1))  # величина, увеличивающая или уменьшающая входной сигнал, подаваемый на функцию активации
        for i in range(0,self.F):
            self.weights[i,:,:,:] = np.random.normal(loc=0, scale=np.sqrt(1./(self.C*self.K*self.K)), size=(self.C, self.K, self.K))

        self.p = padding
        self.s = stride
        self.lr = learning_rate  # Коэффициент скорости обучения
        self.name = name
    ...
    def forward(self, inputs):
        ...
        C = inputs.shape[0]
        W = inputs.shape[1]+2*self.p  # отступ по ширине
        H = inputs.shape[2]+2*self.p  # отступ по высоте
