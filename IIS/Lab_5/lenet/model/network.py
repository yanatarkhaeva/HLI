...
class Net:
    def __init__(self):
        # Lenet -- это образец Сверточной нейросети, по которой сделан этот проект
        # input: 28x28
        # conv1: (5x5x6)@s1p2 -> 28x28x6 {(28-5+2x2)/1+1} -- первый слой -- слой свертки
    ...
    def train(self, training_data, training_label, batch_size, epoch, weights_file):
        total_acc = 0
        for e in range(epoch):  # batch backprop -- групповой алгоритм обратного распространения
    ...
    def predict_with_pretrained_weights(self, inputs, weights_file):
        with open(weights_file, 'rb') as handle:
            b = pickle.load(handle, encoding='bytes')
        # print(b)
        self.layers[0].feed(b[0][b'conv1.weights'], b[0][b'conv1.bias'])
        self.layers[3].feed(b[3][b'conv3.weights'], b[3][b'conv3.bias'])
        self.layers[6].feed(b[6][b'conv5.weights'], b[6][b'conv5.bias'])
        self.layers[9].feed(b[9][b'fc6.weights'], b[9][b'fc6.bias'])
        self.layers[11].feed(b[11][b'fc7.weights'], b[11][b'fc7.bias'])
        for l in range(self.lay_num):
